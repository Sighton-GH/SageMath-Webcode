...
2) First Docker build attempt (failed with nested gym path):
```powershell
cd d:\RacerBot\f1tenth_gym_ros
...
3) Rebuild (failed with uv-build missing):
```powershell
docker compose build
```
Error:
...
4) Added build caching and smaller context:
...
7) Launch bridge (background + log capture):
```powershell
...
9) Exposed WebSocket port in docker-compose.yml:
```
ports:
  - "8765:8765"
```
...
11) Verify port mapping and connectivity:
```powershell
docker compose ps
